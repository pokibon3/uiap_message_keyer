...
### メッセージ記録モード
- `SW-A + SW-B`同時長押し（約1.5s）で記録モードを開始します。
...
記録モード中にスイッチを押すことにより、記録を開始します。スイッチを押す前は練習モードです。
  ...
  - `SW-B`でメッセージB録音開始。
- 記録終了:
  - 同じスイッチを押すと確定保存し、通常モードに戻ります。
  - もう一方のスイッチを押すと記録をキャンセルし、通常モードに戻ります。
- 記録訂正:   
　- 訂正符号(HH)入力で、記録中の直前の1文字を削除する。

　※ 記録モード開始後はパドル入力が可能ですが、記録スロットの選択スイッチを押した時点から記録が開始されます。記録をキャンセルしたい場合は、もう一方のスイッチを押すことによりキャンセルできます。  
